...
def pressure_gain_from_water_height(height):
    pressure = (density_water * gravity_acceleration * height) / 1000  
    return pressure

def pressure_loss_from_pipe(pipe_diameter, pipe_length, friction_factor, fluid_velocity):
    lost_pressure = ((-friction_factor) * pipe_length * density_water * fluid_velocity ** 2) / (2000 * pipe_diameter)
...
def reynolds_number(hydraulic_diameter, fluid_velocity):
    reynolds_number = (density_water * hydraulic_diameter * fluid_velocity) / water_viscosity
...
def main():
    ...
    pressure_psi = convert_kilopascals_to_psi(pressure)
